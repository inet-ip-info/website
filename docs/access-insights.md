...
```sh
go run ./cmd/access-insights-history
```

このコマンドは Mackerel API を呼び出して `/var/lib/inet-ip-info/access-insights-history.json` を生成します。日次の `cmd/access-insights` は Mackerel API を呼び出さず、このファイルが存在する場合だけ読み込みます。
